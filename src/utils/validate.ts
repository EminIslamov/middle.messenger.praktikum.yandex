export const validateFirstName = (first_name: string): string | null => {
    ...
    return regex.test(first_name) ? null : 'Имя должно начинаться с заглавной буквы и может содержать только буквы и дефис.';
};

export const validateSecondName = (second_name: string): string | null => {
    const regex = /^[А-ЯA-Z][а-яa-zА-ЯA-Z-]*$/;
    return regex.test(second_name) ? null : 'Фамилия должна начинаться с заглавной буквы и может содержать только буквы и дефис.';
};
